...
class ModelOptNvFp4FusedMoEDispatch(MoEMethodBase):
    ...
    def apply_ep_prefill(
        ...
        # Reshape x to [1, num_tokens, hidden_size] as a single group for grouped quantize
        x_3d = x.unsqueeze(0)
        masked_m_quant = paddle.to_tensor([num_tokens], dtype=paddle.int32)
        x_q, x_q_sf = scaled_fp4_grouped_quantize(x_3d, masked_m_quant, input_global_scale_single)
        # x_q layout: [num_tokens, hidden_packed, 1], x_q_sf layout: [num_tokens, sf_packed, 1]
        # Squeeze the single expert dim to get 2D tensors for dispatch
        x_q_2d = x_q.squeeze(-1)  # [num_tokens, hidden_packed]
        x_q_sf_2d = x_q_sf.squeeze(-1)  # [num_tokens, sf_packed]
        ...
        if self.ep_prefill_runner.num_worst_tokens > 0:
            ...
            if top_k in (4, 8):
                ...
                permute_scale_t = permute_scale.transpose([1, 2, 0])
                ...
                if max_expert_tokens > 0:
                    ...
                    blocked_scale_t = blocked_scale.transpose([1, 2, 0]) if blocked_scale is not None else None
